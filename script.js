document.addEventListener("DOMContentLoaded", function () {
  ...
  const noteToFretOnString = (note, stringRootNote) => {
    const chromatic = [
      "C",
      "C#",
      "D",
    ...
    const startIdx = chromatic.indexOf(stringRootNote);
    const noteIdx = chromatic.indexOf(note);
    if (startIdx === -1 || noteIdx === -1) return -1;
    return (noteIdx - startIdx + 12) % 12;
  };

  const generateChordShape = (rootNote, chordType) => {
    const template = chordShapeTemplates[chordType];
    ...
    const rootString = template.find((pos) => pos.fret === 0);
    if (!rootString) return null; // no root found

    const rootStringNumber = rootString.string;
    const rootFret = noteToFretOnString(
      rootNote,
      stringRoots[rootStringNumber]
    );

    return template.map((pos) => {
      if (pos.mute) {
        return { string: pos.string, mute: true };
      }
      return {
        string: pos.string,
        fret: rootFret + (pos.fret || 0),
        finger: pos.finger,
      };
    });
  };
  const A_Ma7 = generateChordShape("C", "Ma7");
  console.log(A_Ma7);
    ...
    validChords.forEach((chord) => {
      ...
      btn.addEventListener("click", () => {
        ...
        if (isActive) {
          ...
          // Reset all fret notes
          const fretboard = document.querySelector(
            "#guitar-display .fretboard"
          );
  ...
  const fingerColors = {
    1: "#a8dadc", // light blue
    2: "#fbc4ab", // light peach
    3: "#caffbf", // light green
    4: "#ffd6a5", // light orange
  };

  const showGuitarChordShape = (shape) => {
    ...
    shape.forEach((pos) => {
      ...
      if (note) {
        ...
        if (pos.finger) {
          note.style.backgroundColor = fingerColors[pos.finger];
          note.classList.add(`finger-${pos.finger}`);
        }
      }
    });
  };
  ...
  function showChordOnInstruments(root, chordType) {
    ...
    const shape = generateChordShape(root, chordType);
    if (shape) {
  ...
  // showGuitarChordShape(guitarChordShapes["G_Ma7"]);
});
